...
var server = http.createServer(function(req, res) {
  if (req.url === '/') {
    ...
    request('http://api.population.io:80/1.0/population/1980/' + country1 + '/',
      (error, response, body) => {
        if (error || response.statusCode != 200) {
          console.error(error.message);
          res.end();
        ...
        console.dir(country1Data);

        request('http://api.population.io:80/1.0/population/1980/' + country2 + '/',
          (error, response, body) => {
            if (error || response.statusCode != 200) {
              console.error(error.message);
              res.end();
